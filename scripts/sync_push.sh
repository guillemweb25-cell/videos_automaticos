...
rsync -rtvz --delete --no-o --no-g --no-perms --no-t -e "ssh -p $REMOTE_PORT_SSH" cache/ $REMOTE_SSH:$REMOTE_BASE_PATH/cache/

# 2. Sync YouTube Credentials
echo "Step 2: Synchronizing YouTube credentials..."
rsync -rtvz --delete --no-o --no-g --no-perms --no-t -e "ssh -p $REMOTE_PORT_SSH" backend/youtube_creds/ $REMOTE_SSH:$REMOTE_BASE_PATH/backend/youtube_creds/
