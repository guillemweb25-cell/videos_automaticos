...
class ApiClient {
  ...
  async charGenerate(payload: { description: string; style: string; num_poses: number; seed?: number | null; neutral_bg?: boolean; gender?: string; pose_control?: boolean; name?: string; provider?: string }): Promise<CharGenerateResponse> {
  ...
  async charScene(payload: { character_id: string; prompt: string; num_images: number; width?: number; height?: number; seed?: number | null; pose?: string | null; phone?: boolean; hq?: boolean; preview?: boolean; positive?: string; negative?: string; provider?: string }): Promise<{ ok: boolean; prompt_id?: string; expected?: number; prompt_en?: string; seed?: number; entry_id?: string; positive?: string; negative?: string; preview?: boolean }> {
  ...
  async regenerateParagraph(videoId: number, paragraphId: number): Promise<{ ok: boolean; paragraph_id: number; deleted_images: number; background: boolean }> {
    const res = await fetch(`${this.baseUrl}/videos/${videoId}/paragraphs/${paragraphId}/regenerate`, {
      method: 'POST',
      headers: this.getHeaders(true),
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || 'Error al regenerar el párrafo');
    }
    return res.json();
  }
  ...
  async generateThumbnail(videoId: number, hook?: string, visualPrompt?: string, modelId?: string, generationMode?: string, position?: string, charSide?: string, textAngle?: number): Promise<{ ok: boolean, url: string }> {
  ...
  async updateThumbnailText(videoId: number, hook?: string, position?: string, charSide?: string, textAngle?: number): Promise<{ ok: boolean, url: string }> {
    const res = await fetch(`${this.baseUrl}/videos/${videoId}/update-thumbnail-text`, {
      method: 'POST',
      headers: this.getHeaders(true),
      body: JSON.stringify({ hook, position, char_side: charSide, text_angle: textAngle })
    });
