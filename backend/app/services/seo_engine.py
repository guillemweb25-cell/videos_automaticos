    ...
    def generate_thumbnail_visual_prompt(self, script_snippet: str, style_desc: str, thumbnail_hook: str = "", custom_rules: Optional[str] = None, include_text_in_prompt: bool = False) -> str:
        ...
        if include_text_in_prompt:
            ...
            text_instruction = (
                ...
                "EMOTION (mandatory): the subject's facial expression MUST match the emotional tone of the topic — "
                "worried, sad, anxious or desperate for PROBLEMS (debt, illness, loneliness, fear); hopeful, relieved, "
                "joyful or confident for SOLUTIONS and abundance (money, love, success, health). Do NOT default to a "
                "generic calm smile when the topic is negative. "
                "TOPICAL BACKGROUND (mandatory): the environment filling the empty side must VIVIDLY and dramatically "
                "depict the video's specific topic — e.g. a collapsed devastated city for an earthquake; scattered bills, "
                "a broken piggy bank, an empty wallet or overdue invoices for DEBT; a bright home and symbols of "
                "prosperity for money/abundance; a gym/scale/healthy body for weight loss; an office or interview for a "
                "job. NOT a plain room. Make the background unmistakably about the subject of the video. "
